...
module.exports = {
  //view status
  funViewSitesView: async function (obj, db) {
    try {
      let strLoginUserId = obj.strLoginUserId;


      if (strLoginUserId) {
        let sitesView = await sitesViewSchema.aggregate([
          { $match: {} },
          {
            $project: {
              siteCode: 1,
  ...
  funUpdateAdminModules: async function (obj, db) {
    try {
      ...
      let title = obj.title;
      let icon = obj.icon;
      let sortNo = parseInt(obj.sortNo);
      let arrSubModule = obj.arrSubModule;
      let strActiveStatus = obj.strActiveStatus;
      let araySub = [];

      arrSubModule
        ? arrSubModule.forEach((element) => {
  ...
  funGetAdminModules: async function (obj, db) {
    try {
      ...
      let match = { $and: [query1, query2, query2] };
      let getUserModules = await db
        .collection(config.ADMIN_MODULES_COLLECTION)
        .find(match)
        .toArray();
  ...
  //get admin modules by id
  funGetAdminModulesById: async function (obj, db) {
    try {
      let fkModuleId = obj.fkModuleId;
  ...
  funValidate: async function (obj, db) {
    try {
      let strLoginUserId = obj.strLoginUserId;
